...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "ja": {
        ...
        "over_size_limit": "🔒 投影サイズ {pw} × {ph} は無料範囲（{w}st以下）超過。図案PNGは透かし入り、PDFは利用不可になります。",
...
""",
    ...
    "en": {
        ...
        "over_size_limit": "🔒 Projected size {pw} × {ph} exceeds free tier ({w}st max). Chart PNG will be watermarked, PDF unavailable.",
        "over_color_limit": "🔒 Above {c} colors: chart PNG watermarked, PDF unavailable.",
        "preview_only_notice": "🔒 Current size/colors exceed free tier ({w}st × {c} colors). Preview PNG free DL, chart PNG with watermark, PDF requires [unlock code]({url}).",
