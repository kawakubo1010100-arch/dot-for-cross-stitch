...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "ja": {
        ...
        "free_tier_summary": "💡 無料：{w}ステッチ × {c}色まで全機能（PDF含む）。それ以上は図案に透かしが入り、PDFは利用不可（プレビューPNG・図案PNG[透かし入り]はDL可）",
        "over_size_limit": "🔒 {w}ステッチを超えると、図案PNGに透かし入り＆PDF利用不可になります。",
        "over_color_limit": "🔒 {c}色を超えると図案PNGに透かし、PDFは利用不可になります。",
        "preview_only_notice": "🔒 現在のサイズ・色数は無料範囲（{w}st × {c}色）を超えています。プレビューPNGは無料DL、図案PNGは透かし入りでDL可、PDFは[解除コード]({url})が必要です。",
        "with_watermark": "透かし入り",
        "pdf_paid_only": "PDFは解除コードで利用可能になります",
        "share_section": "シェア",
        "share_to_x": "Xで共有",
...
""",
    ...
    "en": {
        ...
        "unlock_intro": "Get your unlock code on [Gumroad]({url}) ($9 one-time, lifetime access)",
        "unlock_code_label": "Unlock code",
        "unlock_button": "Unlock",
        "unlock_wrong": "Wrong code",
        "free_tier_summary": "💡 Free: up to {w} stitches × {c} colors with full features (incl. PDF). Above that, chart is watermarked and PDF unavailable (preview/chart PNG still downloadable).",
        "over_size_limit": "🔒 Above {w} stitches: chart PNG watermarked, PDF unavailable.",
        "over_color_limit": "🔒 Above {c} colors: chart PNG watermarked, PDF unavailable.",
        "preview_only_notice": "🔒 Current size/colors exceed free tier ({w}st × {c} colors). Preview PNG free DL, chart PNG with watermark, PDF requires [Pro unlock]({url}).",
        "with_watermark": "watermarked",
        "pdf_paid_only": "PDF available with unlock code",
        "share_section": "Share",
